...
3. Spawn one `implementer` subagent with the full plan and the worktree path
...
7. If verify **fails**: call `ExitWorktree` with cleanup (drop, no merge), report failure to user
...
7. **If none pass**: call `ExitWorktree` on all 3, report failure to user — stop
...
Run: `bash ~/.claude/scripts/verify.sh <worktree-path>`
...
- Never merge a worktree that did not pass verification
