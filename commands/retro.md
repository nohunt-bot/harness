...
3. **Decision records**: list decisions made where a real alternative was
...
4. **Lessons**: did the harness mislead you, or did a preventable failure
   ...
   No incident → "none".
5. **Task-file closeout**: if `docs/tasks/` holds a file for this task (see
   `~/.claude/harness/rules/long-tasks.md`), append the final progress line
   and set `Status: done` (or `abandoned` plus why).
6. **Follow-ups**: things noticed but out of scope — one line each, so the
